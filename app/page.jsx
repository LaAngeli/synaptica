...
export default function Home() {
  ...
  const phases = t("home.phases") || [];
  const statTags = t("home.statTags") || [];
  const kitTags = t("home.kitTags") || [];
      ...
      <section className="relative overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-br from-white/90 via-white/85 to-slate-100/90 px-6 py-12 shadow-2xl shadow-slate-200 sm:px-10 lg:px-12">
        ...
        <div className="relative grid gap-10 lg:grid-cols-[1.05fr,0.95fr] lg:items-center">
          ...
          {/* <div className="relative">
            <div className="absolute -left-10 top-6 h-32 w-32 rounded-full bg-[#cdb360]/30 blur-3xl" aria-hidden />
            <div className="absolute -right-10 bottom-8 h-32 w-32 rounded-full bg-[#aa995a]/25 blur-3xl" aria-hidden />
            <div className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white/80 p-6 shadow-xl shadow-slate-200 backdrop-blur-xl">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm uppercase tracking-[0.22em] text-slate-600">{t("home.flowLabel")}</p>
                  <p className="text-xl font-semibold text-slate-900">{t("home.flowTitle")}</p>
                </div>
              </div>
              ...
              <div className="mt-6 space-y-3">
                {phases.map((phase, index) => (
                  <div
                    key={phase.title}
                    className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4 shadow-inner shadow-slate-200/80"
                  >
                    <div className="grid w-full grid-cols-[1fr_auto] items-start gap-3 text-sm text-slate-800">
                      <span className="min-w-0 text-left">
                        {phase.title}
                      </span>

                      <span className="max-w-30 mt-1 text-right text-[11px] uppercase tracking-[0.14em] text-[#817e32] leading-tight sm:text-xs sm:tracking-[0.16em]">
                        {phase.sprint}
                      </span>
                    </div>
                    <div className="mt-3 h-2 w-full rounded-full bg-slate-200/80">
                      <div
                        className="h-2 rounded-full bg-gradient-to-r from-[#cdb360] to-[#9f8a3f]"
              ...
              {/* <div className="mt-8 rounded-2xl border border-slate-200 bg-slate-50/80 p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {kitTags.map((tag) => (
                    <span
                      key={tag}
                      className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-slate-800"
                    >
                      {tag}
                    </span>
                  ))}
